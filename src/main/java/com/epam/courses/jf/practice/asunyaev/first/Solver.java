...
public class Solver implements ISolver {
    ...
    /**
     * Реализация метода для юнит-тестирования задания №3.
     *
     ...
     *
     * Формат выходных данных:
     * В результате выполнения метода task3 в выходной поток должна быть выведена средняя длина (округленная до целого в меньшую сторону)
     * и строки, удовлетворяющие условию. Порядок появления строк в выходном потоке должен совпадать с порядком их появления во входном.
     ...
     * число различных символов минимально. Символы верхнего и нижнего регистра считать различными.
     ...
     * состоящих только из символов латинского алфавита и содержащих равное количество гласных
     * и согласных букв (количество различных вхождений одной буквы в слове учитывается).
     */
    @Override
    public void task5() {
        Scanner scan = new Scanner(System.in);
    ...
    /**
     * Реализация метода для юнит-тестирования задания №9.
     *
     ...
     * соответствующие строкам матрицы. Внутри строки элементы матрицы разделяются знаком табуляции ('\t').
     */
    @Override
    public void task9() {
        Scanner scan = new Scanner(System.in);
    ...
    /**
     * Реализация метода для юнит-тестирования задания №10.
     *
     ...
     *
     * Формат входных данных:
     * Во входном потоке последовательно расположены три целочисленных коэффициента A, B и C,
     * задающие уравнение в общем виде.
     *
     * Формат выходных данных:
     ...
     * В результате выполнения метода task10, в выходной поток посылается строка,
     * соответствующая найденному решению. Значения корней округляются до 2 знаков после запятой.
     * Использовать округление BigDecimal.HALF_UP. В качестве десятичного разделителя используется точка.
     */
    @Override
    public void task10() {
        Scanner scan = new Scanner(System.in);
        ...
        if (d == 0) {
            ...
            System.out.println("Two solutions: "
                    + solution1.setScale(2, RoundingMode.HALF_UP ).stripTrailingZeros()
                    + ", "
                    + solution2.setScale(2, RoundingMode.HALF_UP ).stripTrailingZeros());
        } else {
